...
<a href="{{ route('advisor.negotiations.create') }}" class="btn btn-info">Registrar</a>
<a title="generar PDF" href="negotiation/pdf?name={{$request}}" class="btn btn-danger">GENERAR PDF <i class="fa fa-file-pdf-o"></i></a> 
</div>
{!! Form::open(['route'=>'advisor.negotiations.index','method'=>'GET','class'=>'navbar-form pull-right']) !!}
		...
		@foreach($negotiations as $negotiation)  
           @if($negotiation->estado != "ganada")            
            <tr>
                ...
                <td>
                    <a title="ver detalle" href="{{ route('advisor.negotiations.show',$negotiation->id) }}" class="btn btn-success btn-xs">
                       <i class="fa fa-eye"></i>    
                    </a>                 
                    <a title="modificar" href="{{ route('advisor.negotiations.edit',$negotiation->id) }}" class="btn btn-warning btn-xs">
