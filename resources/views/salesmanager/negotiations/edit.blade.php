@extends('layouts.app')
@section('title','Editar negociacion')
@section('content')
<body style="background-color:#E0F2F0">
<div class="panel panel-default">    
    <div class="panel-heading" style="background-color:#8BCEE5; color:white"><h2><i class="fa fa-briefcase"></i> Editar negociación <font style="text-transform: capitalize;">{{$negotiation->client->estado}} {{$negotiation->client->name}}</font>  </h2>   
    </div>
        <div class="panel-body">
        
{!! Form::open(array('route' => ['salesmanager.negotiations.update',$negotiation->id],'id'=>'formulario', 'method' => 'put')) !!}﻿ 
...
<!--            {!! $i = 0; !!}-->
            ...
            @if($product->flag==1)
                @if($product->pivot->cantidad>$product->cantidad)
                 <tr class="selected" id="fila{{$cont}}" style="background-color: #F6A9B2">
                 <td><center><button id="agotado" type="button" class="btn btn-danger btn-xs" onclick="eliminar2({{$cont}});">X</button></center></td>
                 <td><input type="hidden" name="idproducto[]" value="{{$product->id}}">{{$product->name}}</td>
                 <td><input type="hidden" name="cantidad[]" value="{{$product->pivot->cantidad}}">{{$product->pivot->cantidad}}</td>    
...
<!--            {!! $flagy = 0; !!}-->
                @foreach($productoAgotado as $producto)
...
<!--                 {{$subtotal[$cont]=$product->pivot->cantidad*$product->precio_venta}}-->
                ...
                <tr class="selected" id="fila{{$cont}}">
                 <td><center><button type="button" class="btn btn-danger btn-xs" onclick="eliminar2({{$cont}});">X</button></center></td>
                 <td><input type="hidden" name="idproducto[]" value="{{$product->id}}">{{$product->name}}</td>
                 <td><input type="hidden" name="cantidad[]" value="{{$product->pivot->cantidad}}">{{$product->pivot->cantidad}}</td>    
                 <td><input type="hidden" name="precio_venta[]" value="{{$product->precio_venta}}">{{$product->precio_venta}}</td>
                 <td><input type="hidden" id="subtotal{{$cont}}" value="{{$product->pivot->cantidad*$product->precio_venta}}">{{$product->pivot->cantidad*$product->precio_venta}}</td> 
