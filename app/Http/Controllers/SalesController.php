...
//        $negotiations = Negotiation::where('user_id', '=', Auth::user()->id)->where('estado', '=', 'ganada')->Paginate(11);
            ...
            $anio=date("Y");
            $mes=date("m");
        return view('advisor.sales.index')->with('negotiations',$negotiations)->with('request',$request->name)->with("anio",$anio)->with("mes",$mes);  
    }
    public function create(){ 
        ...
        return view('advisor.sales.create')->with('clients',$clients)->with('products',$products)->with('services',$services);
    }
...
//        dd($totalcost);
        return view('advisor.sales.show')->with('negotiation',$negotiation)->with('my_products',$my_products);
    }
    ...
    public function comprobantsales(Request $request){
        ...
        return view('advisor.sales.comprobante')->with('negotiations',$ventas)->with("anio",$anio)->with("mes",$mes)->with("ingreso",$ingreso);
        dd($ventas);
