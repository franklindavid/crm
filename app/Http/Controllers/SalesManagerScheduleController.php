...
class SalesManagerScheduleController extends Controller
{
     public function index(){
//        $advisors = User::where('type', '=', 'advisor')->Paginate(11);
        return view('salesmanager.schedule.index');
    } 
...
//        dd($Schedule);
    ...
    public function update2(Request $request){
        ...
        $Schedule->update();
        flash('se ha modificado el Scheduleo '.$Schedule->asunto.' de forma exitosa!', 'success');
        return redirect()->route('salesmanager.schedule.index');
        
}
    /////////////////////////////////////////////////consultar agenda de otros 
    public function index3($idusuario){  
        $data = array(); //declaramos un array principal que va contener los datos
        $id = Schedule::where('user_id', '=', $idusuario)->lists('id'); //listamos todos los id de los Scheduleos
        $titulo = Schedule::where('user_id', '=', $idusuario)->lists('asunto'); //lo mismo para lugar y fecha
        $fechaIni = Schedule::where('user_id', '=', $idusuario)->lists('fechainicio');
        $fechaFin = Schedule::where('user_id', '=', $idusuario)->lists('fechafin');
        $informacion = Schedule::where('user_id', '=', $idusuario)->lists('informacion');
        $lugar = Schedule::where('user_id', '=', $idusuario)->lists('lugar');/////
        $allDay = Schedule::where('user_id', '=', $idusuario)->lists('todoeldia');
        $background = Schedule::where('user_id', '=', $idusuario)->lists('color');
        $count = count($id); //contamos los ids obtenidos para saber el numero exacto de Scheduleos
