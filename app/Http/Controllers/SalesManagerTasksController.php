...
class SalesManagerTasksController extends Controller
{
...
//        $tasks = Task::where('user_id', '=', Auth::user()->id)->orderBy('prioridad','ASC')->Paginate(11); 
        return view('salesmanager.tasks.index')->with('tasksold',$tasksold)->with('taskstoday',$taskstoday)->with('tasksnext',$tasksnext);
    }
    public function create(){
        $clients = Client::where('user_id', '=', Auth::user()->id)->lists('name','id');
        return view('salesmanager.tasks.create')->with('clients',$clients);
    }
...
//            $task->save();
        ...
        return redirect()->route('salesmanager.clients.tasks',$task->client_id);
//        dd($request->tipo);
    
    }
    public function edit($id){
        $task = Task::find($id);
        return view('salesmanager.tasks.edit')->with('task',$task);        
    }
...
//            $task= new Task($request->all());
        ...
        return redirect()->route('salesmanager.tasks.edit',$id); 
    }
